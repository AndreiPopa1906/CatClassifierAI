...
for i in df:
    if i != 'Plus' and i != 'Row.names' and i != 'Horodateur' and i != 'Race':
        plt.clf()
        heights, bins, a = plt.hist(df[i], bins=len(categories[i]), color=['blue'], edgecolor='black', rwidth=0.8)
        plt.title(i)
        width = bins[1] - bins[0]
        bin_centers = (bins[:-1] + bins[1:]) / 2
        plt.xticks(bin_centers, categories[i])
        for j in range(len(categories[i])):
            plt.text(bins[j] + width / 2, heights[j] + 1, str(heights[j]), ha='center')
        plt.savefig(f"Histograms/DatasetLevel//{i}.png")
    ...
    for i in df:
        if i != 'Plus' and i != 'Row.names' and i != 'Horodateur' and i != 'Race':
            ...
            for j in range(len(race_data[k][i].keys())):
                plt.text(bins[j] + width / 2, heights[j] + 1, heights[j], ha='center')
            plt.savefig(f"Histograms/RaceLevel/{k + i}.png")
